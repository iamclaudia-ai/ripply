...
test('the tally is a real typed table: plain SQL, columnTypes, declared indexes, avg components', async () => {
  ...
  try {
    ...
    expect(rows).toEqual([{ player_id: 42, games: 2, wins: 1, high_score: 310, avg_score: 280 }]);
    ...
    expect(indexes.map((record) => record.indexname)).toContain('ripply_player_stats__high_score');
